...
use baseview::{
    Event, EventStatus, HostHandler, MouseEvent, WindowContext, WindowHandler, WindowSize,
    copy_to_clipboard,
};
use clack_extensions::gui::{GuiApiType, GuiSize, HostGui};
...
pub struct ExampleHostHandler {
    host: NonNull<c_void>,
    host_gui: HostGui,
    destroyed: Rc<Cell<bool>>,
}

impl ExampleHostHandler {
    unsafe fn host_handle(&self) -> Option<HostMainThreadHandle<'_>> {
        if self.destroyed.get() {
            return None;
        }

        unsafe { Some(HostMainThreadHandle::from_raw(self.host.cast())) }
    }

    fn platform_uses_logical_pixels() -> bool {
        let Some(api_type) = GuiApiType::default_for_current_platform() else {
            return false;
        };

        api_type.uses_logical_size()
    }
}

impl HostHandler for ExampleHostHandler {
    fn request_resize(&mut self, size: WindowSize) {
        let Some(host) = (unsafe { self.host_handle() }) else { return };

        let (width, height) = if Self::platform_uses_logical_pixels() {
        ...
        self.host_gui.request_resize(&host, width, height); // TODO: handle error
    }
