...
use crate::Window;
use winapi::um::winuser::{DefWindowProcA, WM_MOUSEMOVE, WM_PAINT, WM_TIMER, WM_KEYDOWN, WM_KEYUP};
...
pub(crate) unsafe fn handle_message(
    ...
    match message {
        ...
        WM_KEYDOWN => {
            win.lock().unwrap().handle_keydown(wparam);
            0
        }
        WM_KEYUP => {
            win.lock().unwrap().handle_keyup(wparam);
            0
        }
