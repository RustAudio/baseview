        ...
        match event {
            ...
            XEvent::ClientMessage(event) {
                ...
                if event.type_ == self.window.xcb_connection.atoms.XdndEnter {
                    ...
                    self.window.drag_n_drop.version = Some(version);

                    let has_more_types = flags - (flags & (u32::max_value() - 1)) == 1;
                    if !has_more_types {
                        let type_list = vec![data[2] as Atom, data[3] as Atom, data[4] as Atom];
                        self.window.drag_n_drop.type_list = Some(type_list);
                    } else if let Ok(more_types) =
                        self.window.drag_n_drop.get_type_list(source_window, &self.window.xcb_connection)
                    {
                        self.window.drag_n_drop.type_list = Some(more_types);
                    }
                    ...
                    let version = self.window.drag_n_drop.version.unwrap_or(5);
                    ...
                    if !accepted {
                        if let Err(_e) = self.window.drag_n_drop.send_status(
                    ...
                    self.window.drag_n_drop.source_window = Some(source_window);
